...
const invision = (function(){
    ...
    const sendMessage = (message: SendMessageConfig | any) => {
        ...
        if (!communicationInfo || !communicationInfo.targetOrigin) {
            console.error('Not hosted inside an InVision iFrame, communication is not initialized or not allowed (based on target origin restrictions).');
            return;
        }
    ...
    return{
        ...
        setAllowedTargetOrigins : setAllowedTargetOrigins
    }
